...
| [M4.5](#m45-web-ui-でリンクの更新削除published_at--memo) | Web UI でリンクの更新削除                |      |            |                                                     |
| [M4.6](#m46-拡張-ui-で日付メモ付き保存)                  | 拡張 UI で日付メモ付き保存               |      |            |                                                     |
...
- **やること**
  - Web 側（`LinksFilterPanel`）
    ...
    - **共有・再現性**: URL クエリに `from/to` を保持する（現状通り）
      - 週/月の境界ズレ防止のため、可能なら **`tz`（IANA time zone）も URL に保持**する
        - 例: `?from=2025-12-14&to=2025-12-20&tz=Asia%2FTokyo`
...
- **やること**
  ...
  - AI 入力はなるべく多くの情報（title/url/domain/published_at/note/tags/description 等）を渡す方針。
