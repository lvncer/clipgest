...
- 主な要素：
  ...
  - 入力 DTO `CreateLinkInput`：
    - `UserID`, `URL`, `Title`, `Description`, `Domain`, `OGImage`, `PageURL`, `Note`, `Tags` など。
...
4. `service.FetchMetadata` で OGP 情報を取得し、title/description/og_image を補完。
