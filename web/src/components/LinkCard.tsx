...
export default function LinkCard({ link }: { link: LinkItem }) {
  ...
  return (
    <Card className="group hover:shadow-md transition-all duration-300 hover:border-primary/50 overflow-hidden bg-card/80 backdrop-blur-sm h-full">
      <div className="flex flex-col h-full">
        <div className="flex flex-col md:flex-row h-full">
          {/* 左側: OGP Image */}
          <div className="relative w-full md:w-1/3 h-48 md:h-auto min-h-[160px] bg-muted border-b md:border-b-0 md:border-r border-border/50 shrink-0">
            {isLoading ? (
          ...
          <div className="flex flex-col w-full md:w-2/3">
            <CardHeader className="pb-2 pt-4 px-4 md:px-5">
              ...
              <CardTitle className="text-lg leading-snug line-clamp-2 group-hover:text-primary transition-colors">
                <Link href={link.url} target="_blank" rel="noopener noreferrer" className="hover:underline decoration-primary/50 underline-offset-4">
                  {displayTitle}
                </Link>
              </CardTitle>
            </CardHeader>
            ...
            <CardFooter className="pt-2 pb-4 px-4 md:px-5 text-xs text-muted-foreground flex justify-between items-center border-t border-border/50 mt-auto bg-muted/20 py-3">
              <div className="flex items-center gap-1.5" title={link.saved_at}>
                ...
                {formatDistanceToNow(new Date(link.saved_at), { addSuffix: true, locale: ja })}
              </div>
              <Link 
                href={link.url} 
                target="_blank" 
